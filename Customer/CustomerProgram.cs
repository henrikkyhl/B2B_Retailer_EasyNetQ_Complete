...
// warehouse.
Task.Factory.StartNew(() => new Customer.Customer(1, 1, "DK").Start());

// I have no warehouse in Sweeden (SE), so this order request will
// be fulfilled by a warehouse in another country. 
Task.Factory.StartNew(() => new Customer.Customer(2, 1, "SE").Start());

// A customer placing an order for a product which is not in stock.
Task.Factory.StartNew(() => new Customer.Customer(3, 100, "DK").Start());
